...
export const AutoMode: React.FC<{setModeLock: React.Dispatch<React.SetStateAction<boolean>>}> = ({setModeLock}) => {
    ...
    return (
        <>
            ...
            <p className="collapse-title text-xl font-medium">Proxy Password: <input className="input input-bordered input-success input-sm w-full max-w-xs text-xl" ref={passwordRef} defaultValue={"impromptu"} /></p>
            ...
            <table  border={1}>
                ...
                <tr className="text-lg">
                    <td>{proxyId} <button className="btn btn-outline btn-info" onClick={() => navigator.clipboard.writeText(proxyId)}>copy</button></td>
                    <td>{proxyPassword} <button className="btn btn-outline btn-info" onClick={() => navigator.clipboard.writeText(proxyPassword)}>copy</button></td>
                </tr>
            </table>
            }
                ...
                {proxy.map( v => {
                    ...
                    <>
                        <tr className="border-2 border-blue-400 text-lg">
                            <td>{v[0]} <button className="btn btn-outline btn-info" onClick={() => navigator.clipboard.writeText(v[0])}>copy</button></td>
                            <td>{v[1]} <button className="btn btn-outline btn-info" onClick={() => navigator.clipboard.writeText(v[1])}>copy</button></td>
                            <td>{v[2]} <button className="btn btn-outline btn-info" onClick={() => navigator.clipboard.writeText(v[2])}>copy</button></td>
                        </tr>
                        <tr className="border-2 border-blue-400 text-lg">
                        <td colSpan={3}>

...
export const Statistics: React.FC<{replaceId: string}> = ({replaceId}) => {
  ...
  useEffect(()=>{
    ...
    const interval = setInterval(async () => {
      ...
      if(result.exist){
        const lostRateData = result.screen.data.filter(v => v.name === "total").map(
          v => {
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            const isoStr = new Date(Date.now() - new Date().getTimezoneOffset()*60*1000).toISOString().split('T')[1]!;
            return {
      ...
      <details>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="損失率" stroke="#82ca9d" dot={<CustomizedDot />} />
          </LineChart>
          </ResponsiveContainer>
        </div>
      </details>
